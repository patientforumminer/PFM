...
################## since we are now not doing it realtime we don't have/need query_words or entity_matrix ##############


#query_words = parsed_json['query_words']
#entity_matrix = parsed_json['entity_matrix']

# run through the json file with all the messages and collect them per thread
...
for item in parsed_json:
	...
	if not threadid in thread_dict:
		thread_dict[threadid] ={'thread_id':threadid,'thread_title':item['thread_title'], 'content':{'message':{},'comments':[]}}
	del message['thread_id']
	del message['thread_title']
	if 'message' in item['type']:
		thread_dict[threadid]['content']['message'] = message
	else:
		thread_dict[threadid]['content']['comments'].append(item)


#threads = parsed_json['threads']
threads = thread_dict.values()
...
#for thread in threads:
	...
	for postid in postidsforthread:
		...
		sentids_for_this_post = []
		if (threadid,postid) in sentids_per_post:
			sentids_for_this_post = sentids_per_post[(threadid,postid)]
		else:
			print (threadid,postid) , 'not found'
		#print(threadid,postid,sentids_for_this_post)
	...
	json_out = open(outfilename.replace(".json",thread['thread_id']+".json"), 'w')
	json.dump(thread,json_out)
	json_out.close()
	num +=1
